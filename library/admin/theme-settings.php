...
function hybrid_settings_page_add_meta_boxes() {
	...
	do_action( 'add_meta_boxes', hybrid_get_settings_page_name(), 10, 2 );
}
...
function hybrid_load_settings_page_meta_boxes() {
	...
	if ( is_array( $supports[0] ) ) {
		...
		/* Load the 'Lead Pages' meta box if it is supported. */
		if ( in_array( 'leadpages', $supports[0] ) )
			require_once( trailingslashit( HYBRID_ADMIN ) . 'meta-box-theme-leadpages.php' );

		/* Load the 'Misc' meta box if it is supported. */
		if ( in_array( 'misc', $supports[0] ) )
			require_once( trailingslashit( HYBRID_ADMIN ) . 'meta-box-theme-misc.php' );

		/* Load the 'Ultimate Offer' meta box if it is supported. */
		if ( in_array( 'ultimate-offer', $supports[0] ) )
			require_once( trailingslashit( HYBRID_ADMIN ) . 'meta-box-theme-ultimate-offer.php' );
